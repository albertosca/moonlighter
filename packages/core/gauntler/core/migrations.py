...
def current_version(db: Any) -> int:
    """Version of the schema currently applied to `db`. 0 when `schema_version`
    doesn't exist yet or holds no row (a pre-migration-system DB, e.g. one that
    already got the old ad-hoc ALTERs but never had this table)."""
...
def _backup(db: Any) -> Path:
    ...
    backups = Path(gauntler_home()) / "backups"
    backups.mkdir(parents=True, exist_ok=True)
    dest = backups / f"pre-migration-{time.strftime('%Y%m%d-%H%M%S')}.db"
...
def run_migrations(db: Any) -> list[str]:
    ...
    runs in its own transaction that bumps schema_version — a failure leaves
    the version at the last fully-applied migration, never a torn state."""
        ...
        for attempt in range(_LOCK_RETRIES):  # pragma: no branch - always breaks or raises
            try:
                ...
                if "locked" in str(e).lower() and attempt < _LOCK_RETRIES - 1:
                    logger.warning("migration %s: DB locked, retrying (%d)", name, attempt + 1)
