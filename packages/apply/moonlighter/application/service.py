...
def _inject_reply_alias(answers: dict[str, str], ref: str, config: dict[str, Any]) -> None:
    ...
    Matches labels via is_email_label -- anchored the same way field_map's own
    email rule is (`^e-?mail`), so "the email field" cannot drift between the
    field that gets auto-filled and the one the alias overwrites. Falls back
    to an "Email" key when no existing field matched, same as the pre-refactor
    inject_email_alias this replaces (see email_alias.py's is_email_label)."""
    base_address = config.get("email", {}).get("address")
    if not base_address:
        return
    alias = build_email_alias(base_address, ref)
    injected = False
    for key in list(answers.keys()):
        if is_email_label(key):
            answers[key] = alias
            injected = True
    if not injected:
        answers["Email"] = alias
