...
When a curated bullet pool exists — `cv-pool.yaml` in `MOONLIGHTER_HOME`, or wherever `cv.pool` in `config.yaml` points — `prepare_application` also tailors your CV to the posting: one LLM call selects and orders bullets from your pool and writes a short summary and a technical-expertise line from your profile (it can also answer "the base CV already fits" and change nothing), and the result renders through your own LaTeX template and compiles with `pdflatex` when installed.
...
- **Your bullets, not the model's.** Every bullet comes from your pool; the model chooses and orders them (translating them for a Portuguese posting), and writes the summary and technical-expertise line under instructions to use only your profile and never inflate a claim. The sheet always tells you to review the generated PDF before uploading.
...
Config keys: `cv.pool`, `cv.template_dir` (holding `cv-template.en.tex`, optionally `cv-template.pt.tex` for Portuguese postings), `cv.generated_dir` (default `cv-generated` under `MOONLIGHTER_HOME`). Defaults for each are in [Configuration](../reference/config.md#cv).
