...
from candidatador.services import apply_service
...
def test_archive_screenshots_swallows_exception(tmp_path):
    src = tmp_path / "456"
    src.mkdir()
    with patch("candidatador.services.apply_service.shutil.move", side_effect=OSError("disk")):
...
async def test_confirm_apply_without_email_config_skips_alias(tmp_db, tmp_path):
    ...
    with (
        patch("candidatador.services.apply_service.browser") as mock_browser,
        patch("candidatador.services.apply_service.detect_applier", new=AsyncMock(return_value=applier)),
        patch("candidatador.services.apply_service.resolve_cv_path", return_value=str(cv)),
    ):
        mock_browser.new_page = AsyncMock(return_value=_page(job.url))
        mock_browser.save_screenshot = AsyncMock()
