...
from candidatador.db import Job, ScanLog, init_db
...
async def test_add_job_fetches_description_when_empty(tmp_db):
    ...
    with (
        patch("candidatador.services.scan_service.httpx.AsyncClient", return_value=acm),
        patch(
...
async def test_add_job_http_non_200_returns_error(tmp_db):
    ...
    with patch("candidatador.services.scan_service.httpx.AsyncClient", return_value=acm):
        result = await scan_service.add_job(
...
from candidatador.scanner.base import RawJob  # noqa: E402
...
async def _run_scan(raws, *, eval_mock=None, linkedin_exc=None, linkedin_jobs=None, config=None):
    ...
    with (
        ...
        patch("candidatador.services.scan_service.evaluate_job", new=eval_mock),
