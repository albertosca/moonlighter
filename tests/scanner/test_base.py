...
def test_rawjob_with_all_fields():
    now = datetime.now()
    job = RawJob(
        source="greenhouse", company="Stripe", title="Eng", url="https://x.com",
        location="Remote", remote_type="remote", description="desc",
        posted_at=now, salary_min=100000, salary_max=150000,
        salary_currency="USD", salary_source="stated",
    )
